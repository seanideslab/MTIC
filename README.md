...
└── output/
    ...
    ├── fig9\_confusion\_matrix\_m4\_10pct.csv
    ├── fig10\_threshold\_sweep\_heatmap.csv
    ├── fig10\_fpr\_leadtime\_scatter.csv
...
This repository uses separate licenses for code and dataset artifacts:

* `LICENSE` — MIT License for the C source code and build scripts.
* `LICENSE-CSV` — CC BY 4.0 for the generated CSV output files under `output/`.

The licensing split is intended to make reviewers comfortable with code reuse while still keeping the CSV dataset outputs openly reusable.
...
|`output/fig9\_confusion\_matrix\_m4\_10pct.csv`|Fig. 9|Confusion-matrix support for `M=4` with 10% corruption|
|`output/fig10\_threshold\_sweep\_heatmap.csv`|Fig. 10(a)|Threshold sweep heatmap for FPR / lead-time tradeoffs|
|`output/fig10\_fpr\_leadtime\_scatter.csv`|Fig. 10(b)|FPR versus lead-time scatter data|
